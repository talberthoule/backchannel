...
a text model; configuring a self-hosted endpoint does not enable it.
...
model, and `REFINEMENT_MODEL` is reached only when the row has no model set. The card also auto-retries its summary fetch so it recovers on
