...
Git worktrees live **outside** this checkout, at `C:/work/backchannel/<lane>`: `git worktree add C:/work/backchannel/<lane> -b agent/alp-NNN-slug master`. The main checkout is `C:/code/backchannel`, outside OneDrive. The earlier checkout sat in a OneDrive-synced folder, and sync repeatedly stripped the `.git` file out of worktrees created under `.worktrees/`, orphaning them from `git worktree list` while leaving the directories behind (ALP-139). Keep checkouts and worktrees out of synced folders, and do not recreate `.worktrees/`. Note also that the Docker Compose stack binds the main checkout, so a worktree suits docs, review, and analysis rather than work needing integration testing.
...
| `transcript_refiner` | text | Interval, default 45s, plus a drain stage; off by default | `backend/app/services/transcript_refiner.py` | Rewrites stored transcript entries (tokenized text only) for punctuation, casing and mishearings; a rewrite is accepted only with the identical token multiset, the original is kept in `raw_text`, and the browser gets a `transcript_updated` message. Also runs before post-import analysis |
...
Live call audio (mixed mic + system tracks) is appended to `DATA_DIR/audio/<session_id>/segment_<n>.wav` (`backend/app/services/audio_store.py`, `call_segments.audio_path`). Batch transcription routes through `create_transcriber` (`backend/app/services/local_transcriber.py`): `local-*` model ids run ONNX Whisper/Parakeet locally via `onnx-asr` (weights download to `DATA_DIR/asr-models/` on first use), everything else goes to Gemini. Text LLM calls route through `backend/app/services/llm.py` by the model's registry provider. The workspace meeting language (`transcription.language` app setting, `auto` or an ISO 639-1 code; list and prompt hint in `backend/app/services/transcription_language.py`) rides on `TranscriptionRuntimeConfig.language` into `create_transcriber` and the live gateways (ALP-399). Parakeet takes no language: v2 (`local-parakeet-tdt-0.6b`) is English-only and v3 (`local-parakeet-tdt-0.6b-v3`, loaded int8 via `LOCAL_MODEL_QUANTIZATION`) detects 25 European languages. `local-whisper-large-v3-turbo` (onnx-community export, loaded uint8 because the int8 export's ConvInteger has no CPU kernel in onnxruntime 1.21) covers the rest, batch only (ALP-406). Local ASR registry entries carry a `languages` list, and fit-test recommendations only badge models that suit the meeting language (`recommendable_for_language`, ALP-405); with a set language a model listing it wins, otherwise the higher `multilingual_rank`.
...
- Re-transcription: `POST /api/sessions/{id}/retranscribe` replays stored segment audio through any batch-capable model (destructive to existing transcript entries; it deletes them before replaying, so it verifies a local model's runtime first via `local_asr_status()` and refuses with 503 rather than erasing a transcript it cannot rebuild - ALP-376); `GET /api/sessions/{id}/segments/{n}/audio` serves the recorded WAV
...
- Ask (live): `POST /api/sessions/{id}/ask` answers one question against the running call's transcript, live insights, strategic signals, directives, and stored document summaries, then saves the answer as a starred `asked` insight. Separate from `/api/chat`: single-session, stateless, small recency-first budget. Document summaries are the persisted `documents.summary` values (ALP-181/ALP-192) read straight from the database - never a summarization call; documents without one list as filenames only
- PII Shield: `GET/PUT /api/pii-shield` (settings, coverage, on-device model state), `POST /api/pii-shield/preview`, `POST /api/pii-shield/ner/install`, and per session `GET /api/sessions/{id}/pii/summary` (counts), `GET /api/sessions/{id}/pii` (ledger with values, audited), `POST /api/sessions/{id}/pii/protect` (re-protect stored text). Personal data is tokenized at every text ingress (`backend/app/services/pii/shield.py`, `protect_text` / `protect_name`) so stored rows and every model prompt hold `[PERSON_1]`-style tokens; the encrypted per-session vault is `pii_vault_entries`, and values are put back only at the edge: `PiiRevealMiddleware` for session-scoped JSON, `RevealingWebSocket` for live messages, `?reveal=1` on exports, and `/api/chat`. Nothing under `services/agents` or `services/llm.py` may import `reveal_text`. Audio is enforced, not advised: `transcription_runtime.audio_lock_reason` locks the batch transcriber and the live gateway to local models while the shield is on (the orchestrator's `audio_local_only` flag; blocked reason `pii_shield`), while cloud text models stay admitted because they receive tokens only. See docs/configuration.md "PII Shield"
- Model downloads: `GET /api/model-downloads` reports every set of model weights the process has fetched (`queued`, `downloading` with bytes and percent, `installed`, `error`), with `POST /api/model-downloads/{key}/retry` and `DELETE /api/model-downloads/{key}`. Producers register through `backend/app/services/model_downloads.py`; the PII NER fetch reports exact bytes through `hf_hub_download`'s `tqdm_class`, the onnx-asr fetch by polling its target directory. The ingest path never triggers or waits on a download
