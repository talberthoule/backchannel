...
$migration = Join-Path $scriptsRoot "migrate_releases_to_r2.ps1"
$common = Join-Path $scriptsRoot "r2-release-common.ps1"
. $common

$migrationText = Get-Content -Raw -LiteralPath $migration
Assert-True (
    $migrationText.Contains('. (Join-Path $PSScriptRoot "r2-release-common.ps1")')
) "Migration script does not dot-source the shared R2 helper"
...
foreach ($name in @("Invoke-R2", "Assert-R2Success", "Get-RemoteLatest")) {
    ...
    Assert-True ($null -eq $definition) "Moved function still defined by migration: $name"
}
foreach ($name in @("Test-ExactSet")) {
    $definition = $ast.Find(
        {
            param($node)
            $node -is [Management.Automation.Language.FunctionDefinitionAst] -and
...
try {
    ...
    $success = Invoke-R2Object $script:R2Client "head" "--bucket" $script:Bucket "--key" "releases/latest.json"
    ...
    Assert-True ($ErrorActionPreference -eq "Stop") "Invoke-R2Object did not restore ErrorActionPreference"
    Assert-True $PSNativeCommandUseErrorActionPreference "Invoke-R2Object did not restore PSNativeCommandUseErrorActionPreference"
    ...
    try {
        $null = Invoke-R2Object $script:R2Client "head" "--bucket" $script:Bucket "--key" "releases/latest.json"
    } catch {
    ...
    Assert-True $restoredContinuePreference "Invoke-R2Object did not restore a Continue ErrorActionPreference"

    $env:R2_FAKE_RESULT = "missing"
    $missing = Invoke-R2Object $script:R2Client "get" "--bucket" $script:Bucket "--key" "releases/latest.json" "--output" "ignored.json"
    ...
    $latest = Get-R2Latest (Join-Path $temporary "latest.json") $script:Bucket $script:R2Client
    ...
    $denied = Invoke-R2Object $script:R2Client "get" "--bucket" $script:Bucket "--key" "releases/latest.json" "--output" "ignored.json"
    ...
    $failedClosed = $false
    try {
        Get-R2Latest (Join-Path $temporary "latest.json") $script:Bucket $script:R2Client
    } catch {
