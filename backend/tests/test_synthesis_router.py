...
class SynthesisRouterTests(unittest.IsolatedAsyncioTestCase):
    async def test_history_is_returned_only_for_the_explicit_opt_in(self):
        # Signals reach the live call view as insight rows (ALP-308), so the
        # raw history stays behind the opt-in the post-call panel uses.
        session_id = uuid4()
        db = AsyncMock()
        ...
        self.assertEqual(1, default.signal_history_count)
        self.assertEqual([], default.signal_history)
        self.assertEqual(1, explicit.signal_history_count)
        self.assertEqual(expected.signal_history, explicit.signal_history)
    ...
    def test_websocket_payload_sends_the_count_not_the_rows(self):
        ...
        # The rows travel as insights now, so the socket carries only the count.
    ...
    async def test_live_refresh_dispatches_to_strategic_signals(self):
        ...
        with (
            patch(
                ...
                # The live cycle returns (synthesis, signal insight rows).
                new=AsyncMock(return_value=(expected, {"created": [], "updated": []})),
            ) as signals,
            patch(
