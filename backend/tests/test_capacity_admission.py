...
def _diarizer(
    ...
    # Mirror assess_fit_record: both of these are required_fields for the
    # Sortformer record, so a record missing either one classifies Incompatible.
    # Without this the helper can build a "current" record with holes in it,
    # which the runtime cannot produce and which tests should not assert on.
    if contention_rtf is None or peak_memory_mb is None:
...
class AssessCallCapacityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_an_invalid_sortformer_record_is_excluded_and_named(self):
        ...
        An invalid input is missing, not passing: it must never be rounded up
        into a favorable verdict.
        """
        assessment = await self._assess(
            fit_result=None,
            rows=[],
            diarizer=_diarizer(validity="incompatible"),
        )

        self.assertNotIn("diarization_sortformer", assessment.modelled)
        self.assertTrue(
            any("current benchmark" in item for item in assessment.not_modelled)
        )

    async def test_a_lightweight_machine_is_not_told_about_sortformer_validity(self):
        """Message precedence: the honest message beats the irrelevant one.

        A machine on the lightweight diarizer needs no Sortformer benchmark, so
        complaining that its record is incompatible answers a question nobody
        asked and buries the real one.
        """
        assessment = await self._assess(
            fit_result=None,
            rows=[],
            diarizer=_diarizer(
                effective=DIARIZER_LIGHTWEIGHT, validity="incompatible"
            ),
        )

        self.assertTrue(
            any("lightweight diarizer" in item for item in assessment.not_modelled)
        )
        self.assertFalse(
            any("current benchmark" in item for item in assessment.not_modelled)
        )
    ...
    async def test_an_incomplete_benchmark_is_named_as_unmeasured(self):
        """The message is now the validity reason, not a separate stale note.

        ALP-160 classifies a record missing either required number as
        Incompatible, so the validity branch reports it and the old
        completeness branch became unreachable.
        """
        expected = (
            "diarization: This machine has not been measured against the "
            "current benchmark."
        )
